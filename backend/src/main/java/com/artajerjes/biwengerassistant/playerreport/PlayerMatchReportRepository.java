...
public interface PlayerMatchReportRepository
  ...
  Optional<PlayerMatchReport> findFirstByPlayer_BiwengerPlayerIdAndSeasonAndRoundShortAndPointsIsNotNullOrderByMatchDateDesc(
      String biwengerPlayerId,
      String season,
      String roundShort);
  ...
  @Query("""
      ...
      WHERE r.biwengerRoundId = :biwengerRoundId
        AND r.season IS NOT NULL
        AND TRIM(r.season) <> ''
      """)
  List<String> findSeasonsByBiwengerRoundId(
      @Param("biwengerRoundId") Long biwengerRoundId);
  ...
  @Query(value = """
      ...
      ORDER BY r.player_id, r.match_date DESC
      """, nativeQuery = true)
  List<PlayerMatchReport> findTop10ScoredReportsByPlayerIds(
      @Param("playerIds") List<Long> playerIds);
  ...
  @Query("""
      ...
      WHERE p.league.id = :leagueId
        AND r.season = :season
        AND r.biwengerRoundId IS NOT NULL
        AND r.roundShort IS NOT NULL
        AND TRIM(r.roundShort) <> ''
      ORDER BY r.matchDate ASC, r.biwengerRoundId ASC
      """)
  List<PlayerMatchReport> findRoundReportsByLeagueAndSeason(
      @Param("leagueId") Long leagueId,
      @Param("season") String season);
}